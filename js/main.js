...
const respondToAction = (scenario, allData, type, func = () => { }) => {
    ...
    switch (type) {
        ...
        case 'continue':
            ...
            if (typeof scenario.rewards !== 'undefined')//make sure there is value to add and display correctly
            {
                if (tempMoney.length > 1)
                    ...
                    str = (thousand && parseInt(scenario.rewards.money) > 0) ? (parseInt(tempMoney * 1000) + parseInt(scenario.rewards.money)) / 1000 : parseInt(tempMoney * 100) / 1000;
            }
            else
                str = tempMoney;
